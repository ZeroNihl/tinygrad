...
class TestBlake3(unittest.TestCase):
  ...
  def test_vectors(self):
    for length, expected_hex in self.TEST_VECTORS.items():
      with self.subTest(length=length):
        input_data = bytes(i % 251 for i in range(length))
        input_tensor = Tensor(input_data, dtype=dtypes.uint8)
        hash_output = blake3(input_tensor).realize().numpy().tobytes().hex()
